...
interface AuthContextType {
  ...
  diagnosticLog: string[]; // <-- ADDED for on-screen debugging
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    addDiagnosticLog("AuthProvider mounted. Initializing...");

    const services = initializeFirebaseServices(addDiagnosticLog);
    if (!services) {
      addDiagnosticLog("Firebase services failed to initialize.");
      setIsFirebaseError(true);
      setIsReady(true);
      setIsLoadingRental(false);
      return;
    }
    addDiagnosticLog("Firebase services object created.");
    setFirebaseServices(services);
    setIsFirebaseError(false);

    addDiagnosticLog("Setting up onAuthStateChanged listener...");
    const unsubscribeAuth = onAuthStateChanged(services.auth, (user) => {
      addDiagnosticLog(user ? `onAuthStateChanged: User found (UID: ${user.uid})` : "onAuthStateChanged: No user found.");
      setFirebaseUser(user);
      if (!user) {
        setFirestoreUser(null);
        setActiveRental(null);
      }
      addDiagnosticLog("Setting isReady to true.");
      setIsReady(true);
    });
    addDiagnosticLog("onAuthStateChanged listener attached.");

    return () => {
      addDiagnosticLog("AuthProvider unmounted. Cleaning up auth listener.");
      unsubscribeAuth();
    };
  }, []);
 ...
 const endRental = async (returnedToStallId: string) => {
  ...
  const value: AuthContextType = {
    ...
    logMachineEvent,
    firebaseServices,
    diagnosticLog, // <-- ADDED for on-screen debugging
  };
