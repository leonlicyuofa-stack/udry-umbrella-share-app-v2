...
exports.createPaymePayment = onCall({ secrets: ["PAYME_APP_ID", "PAYME_APP_SECRET"], invoker: "public", cors: true }, async (request) => {
    ...
    const PAYME_SANDBOX_BASE = 'https://api.test.payme.hsbc.com.hk';
    ...
    if (!PAYME_APP_ID || !PAYME_APP_SECRET) {
        logger.error("PayMe Error: Server is missing PAYME_APP_ID or PAYME_APP_SECRET.");
    ...
    try {
        ...
        const tokenResponse = await fetch(`${PAYME_SANDBOX_BASE}/v2/oauth2/token`, {
            method: 'POST',
            headers: {
        ...
        if (!tokenResponse.ok || !tokenData.access_token) {
            logger.error('PayMe Step 2 FAILED: Could not get access token.', { status: tokenResponse.status, data: tokenData });
            throw new HttpsError('internal', `Failed to authenticate with PayMe: ${tokenData.error_description || 'Unknown error'}`);
        }
        const accessToken = tokenData.access_token;
        logger.info("PayMe Step 2 SUCCESS: Access token received.");
        ...
        logger.info(`PayMe Step 3: Sending payment creation request for ${merchantReference}.`);
        
        const paymentResponse = await fetch(`${PAYME_SANDBOX_BASE}/v2/payments`, {
            method: 'POST',
            headers: {
        ...
        if (!paymentResponse.ok || !paymentData.paymeLink) {
            logger.error('PayMe Step 3 FAILED: Could not create payment request.', { status: paymentResponse.status, data: paymentData });
            const errorMessage = `PayMe API Error: ${paymentData.errorDescription || 'Unknown error'} (Code: ${paymentData.errorCode})`;
            throw new HttpsError('internal', errorMessage);
        }

        logger.info(`PayMe Step 3 SUCCESS: Received paymeLink for ${merchantReference}.`);
        return { success: true, paymeLink: paymentData.paymeLink };
