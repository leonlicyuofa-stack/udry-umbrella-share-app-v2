...
interface AuthContextType {
  ...
  isReady: boolean;
  loading: boolean;
  isVerified: boolean; // NEW: Expose the final verification status
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // New state to track the final verification status
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(services.auth, (user) => {
      ...
      if (!user) {
        ...
        setIsVerified(false); // Logged out user is not verified
      }
      setIsReady(true);
    });
  ...
  useEffect(() => {
    ...
    // --- Grandfather clause logic moved here ---
  ...
  const signUpWithEmail = async ({ name, email, password }: SignUpFormData) => {
    ...
    try {
      ...
      // We no longer send the email from here, the handler component will do it.
